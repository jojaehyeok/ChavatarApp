...
function DateFilterStrip({ filterDate, upcomingDates, onSelect, theme }: DateFilterStripProps) {
  ...
  // 오늘부터 한 달(30일)간 칩으로 표시 — 넘치면 가로 스크롤로 드래그해서 봄.
  // 예전엔 "이번주 일~토" 고정 주간이라, 예를 들어 금요일에 보면 모레(일요일)가 "다음주"로
  // 취급되어 목록에 실제 예약이 있는데도 칩에서 통째로 빠지는 버그가 있었음
  const weekStart = new Date(today);
  const weekEnd = new Date(today);
  weekEnd.setDate(today.getDate() + 29);
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.tabBar }]}>
      ...
      <View style={[styles.container, { backgroundColor: theme.background }]}>
          ...
          <Pressable style={styles.drawerOverlay} onPress={closeDrawer}>
            <Animated.View style={[styles.drawer, { backgroundColor: theme.card, paddingTop: insets.top, transform: [{ translateX: drawerAnim }] }]}>
              ...
              <View style={[styles.drawerFooter, { paddingBottom: Math.max(insets.bottom, 16) }]}>
                <Text style={[styles.drawerFooterText, { color: theme.textSub }]}>v1.3.17</Text>
              </View>
            </Animated.View>
          </Pressable>
        </Modal>
            ...
            <View style={[styles.modalContent, { backgroundColor: theme.card }]}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  const item = moreOptionsItem!;
                  setMoreOptionsItem(null);
                  setTimeout(() => openRequestInfo(item), 300);
                }}
              >
        ...
        <Modal visible={!!requestInfoItem} transparent animationType="slide">
          <Pressable style={styles.modalOverlay} onPress={() => setRequestInfoItem(null)}>
            <Pressable style={[styles.modalContent, { backgroundColor: theme.card }]}>
              <View style={[styles.modalHandle, { backgroundColor: isDark ? '#444' : '#ddd' }]} />
              <Text style={[styles.modalTitle, { color: theme.textMain }]}>요청사항 확인</Text>

              <Text style={[styles.label, { marginTop: 4 }]}>딜러정보</Text>
              ...
              <View style={{
                ...
                <Text style={{ fontSize: 14, color: theme.textMain, lineHeight: 20 }}>
