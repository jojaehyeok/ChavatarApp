...
interface RawBooking {
  ...
  // 묶음 진단 — 한 번 이동해서 여러 대를 보는 구조라 오지/긴급 추가금이 붙지 않는다.
...
// 등급별 차등은 9월부터. 8월은 등급 구분 없이 규정표 그대로 적용한다.
// 대시보드 settlement.tsx의 TIER_SPLIT_FROM / BONUS_BEFORE_TIER_SPLIT와 같은 값이어야
// 앱에 보이는 금액과 실제 지급액이 어긋나지 않는다.
...
const effectiveRemoteBonus = (
  ...
  // 묶음 건에는 오지/긴급 추가금이 붙지 않는다 — 한 장소로 한 번 이동해서 여러 대를
  // 보는 구조라서다(대시보드 settlement.tsx의 effectiveRemoteBonus와 같은 기준).
  ...
  const rate =
    visitedAt < TIER_SPLIT_FROM
      ...
      : BONUS_BY_TIER[tier] ?? BONUS_BY_TIER.general;
